...
def add_me_to_the_queue(x_queue: List[str], n_queue: List[str], ticket_type: int, name: str) \
     -> List[str]:
    ...
    return (x_queue if ticket_type else n_queue) + [name]
...
def sorted_names(queue: List[str]) -> List[str]:
    ...
    queue_copy = queue[:]
    queue_copy.sort()
    # the function sorted can also be used tom return a copy:
        #return sorted(queue)
    return queue_copy
