...
def getPal(min_factor: int, max_factor: int, high_low: bool) -> int:
    ...
    for n in range(*args):
        str_n = str(n)
        if str_n == str_n[::-1] and any(factor[1] <= max_factor for factor in getFactors(n, min_factor, max_factor)):
            return n
    return 0
...
def largest(min_factor: int, max_factor: int) -> Tuple[Optional[int], List[List[int]]]:
    ...
    return (number, getFactors(number, min_factor, max_factor)) if number \
        else (None, getFactors(number, min_factor, max_factor))
