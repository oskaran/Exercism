...
def is_criticality_balanced(temperature: Union[int, float], neutrons_emitted: \
...
def reactor_efficiency(voltage: Union[int, float], current: Union[int, float], \
    ...
    """
    eta = ((voltage * current) / theoretical_max_power) * 100
    if eta >= 80      :
        return 'green'
    if 60 <= eta < 80 :
        return 'orange'
...
def fail_safe(temperature: Union[int, float], neutrons_per_sec: Union[int, float], \
    ...
    if (temperature * neutrons_per_sec) < (0.9 * threshold):
        return 'LOW'
    if (0.9 * threshold) <= (temperature * neutrons_per_sec) <= (1.1 * threshold):
        return 'NORMAL'
